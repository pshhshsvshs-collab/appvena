        ...
        if (documentToView.type === 'invoice') {
            ...
            return (
                <div id="invoice-content" className="p-1">
                    <div className="printable-content bg-slate-50 font-sans text-slate-800 printable-area">
                        <div className="max-w-4xl mx-auto bg-white p-4 sm:p-6 lg:p-8 xl:p-12 shadow-lg">
                            <header className="flex flex-col sm:flex-row justify-between items-start mb-8 sm:mb-12 space-y-4 sm:space-y-0">
                                <div className="w-full sm:w-auto">
                                    {userProfile.logoBase64 ? (
                                        <img src={userProfile.logoBase64} alt="Company Logo" className="h-16 sm:h-20 max-w-[160px] sm:max-w-[200px] object-contain mb-4" />
                                    ) : (
                                        <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900">{userProfile.companyName}</h1>
                                    )}
                                    <p className="text-xs sm:text-sm text-slate-500">{userProfile.address}</p>
                                    <p className="text-xs sm:text-sm text-slate-500">{userProfile.phone} | {userProfile.email}</p>
                                </div>
                                <div className="text-left sm:text-right w-full sm:w-auto">
                                    <h2 className="text-xl sm:text-2xl font-bold uppercase text-slate-400 tracking-widest">Invoice</h2>
                                    <p className="text-xs sm:text-sm text-slate-500 mt-1">No: <span className="font-semibold text-slate-700">INV-{project.id.slice(-6)}</span></p>
                                    <p className="text-xs sm:text-sm text-slate-500">Tanggal: <span className="font-semibold text-slate-700">{new Date().toLocaleDateString('id-ID')}</span></p>
                                </div>
                            </header>
                            ...
                            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
                                <div className="bg-slate-50 p-4 sm:p-6 rounded-xl">
                                    <h3 className="text-xs font-semibold uppercase text-slate-400 mb-2">Ditagihkan Kepada</h3>
                                    <p className="font-bold text-slate-800">{clientForDetail.name}</p>
                                    <p className="text-xs sm:text-sm text-slate-600">{clientForDetail.email}</p>
                                    <p className="text-xs sm:text-sm text-slate-600">{clientForDetail.phone}</p>
                                </div>
                                <div className="bg-slate-50 p-4 sm:p-6 rounded-xl">
                                    <h3 className="text-xs font-semibold uppercase text-slate-400 mb-2">Diterbitkan Oleh</h3>
                                    <p className="font-bold text-slate-800">{userProfile.companyName}</p>
                                    <p className="text-xs sm:text-sm text-slate-600">{userProfile.email}</p>
                                    <p className="text-xs sm:text-sm text-slate-600">{userProfile.phone}</p>
                                </div>
                                <div className="bg-blue-600 text-white p-4 sm:p-6 rounded-xl printable-bg-blue printable-text-white sm:col-span-2 lg:col-span-1">
                                    <h3 className="text-xs font-semibold uppercase text-blue-200 mb-2">Sisa Tagihan</h3>
                                    <p className="font-extrabold text-2xl sm:text-3xl tracking-tight">{formatCurrency(remaining)}</p>
                                    <p className="text-xs sm:text-sm text-blue-200 mt-1">Jatuh Tempo: {formatDate(project.date)}</p>
                                </div>
                            </section>
                                ...
                                <table className="w-full text-left min-w-full">
                                    <thead>
                                        <tr className="border-b-2 border-slate-200">
                                            <th className="p-2 sm:p-3 text-xs sm:text-sm font-semibold uppercase text-slate-500">Deskripsi</th>
                                            <th className="p-2 sm:p-3 text-xs sm:text-sm font-semibold uppercase text-slate-500 text-center">Jml</th>
                                            <th className="p-2 sm:p-3 text-xs sm:text-sm font-semibold uppercase text-slate-500 text-right">Harga</th>
                                            <th className="p-2 sm:p-3 text-xs sm:text-sm font-semibold uppercase text-slate-500 text-right">Total</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td className="p-2 sm:p-3 align-top">
                                                <p className="font-semibold text-slate-800 text-sm sm:text-base">{project.packageName}</p>
                                                <p className="text-xs text-slate-500 break-words">{selectedPackage?.digitalItems.join(', ')}</p>
                                            </td>
                                            <td className="p-2 sm:p-3 text-center align-top text-sm sm:text-base">1</td>
                                            <td className="p-2 sm:p-3 text-right align-top text-sm sm:text-base">{formatCurrency(selectedPackage?.price || 0)}</td>
                                            <td className="p-2 sm:p-3 text-right align-top text-sm sm:text-base font-semibold">{formatCurrency(selectedPackage?.price || 0)}</td>
                                        </tr>
                                        {project.addOns.map(addon => (
                                            <tr key={addon.id}>
                                                <td className="p-2 sm:p-3 text-slate-600 align-top text-sm sm:text-base">- {addon.name}</td>
                                                <td className="p-2 sm:p-3 text-center align-top text-sm sm:text-base">1</td>
                                                <td className="p-2 sm:p-3 text-right align-top text-sm sm:text-base">{formatCurrency(addon.price)}</td>
                                                <td className="p-2 sm:p-3 text-right align-top text-sm sm:text-base font-semibold">{formatCurrency(addon.price)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ...
                            <section className="mt-8 sm:mt-12">
                                <div className="flex flex-col lg:flex-row justify-between gap-6 lg:gap-8">
                                    <div className="w-full lg:w-2/5">
                                        <h4 className="font-semibold text-slate-600 mb-2">Tanda Tangan</h4>
                                        {project.invoiceSignature ? (
                                            <img src={project.invoiceSignature} alt="Tanda Tangan" className="h-16 sm:h-20 mt-2 object-contain" />
                                        ) : (
                                            <div className="h-16 sm:h-20 mt-2 flex items-center justify-center text-xs text-slate-400 italic border border-dashed rounded-lg">
                                                Belum Ditandatangani
                                            </div>
                                        )}
                                    </div>
                                    <div className="w-full lg:w-2/5 space-y-2 text-sm">
                                        ...
                                        {project.discountAmount && project.discountAmount > 0 && (
                                            <div className="flex justify-between">
                                                ...
                                                <span className="font-semibold text-green-600 print-text-green">-{formatCurrency(project.discountAmount)}</span>
                                            </div>
                                        )}
                                        <div className="flex justify-between">
                                            <span className="text-slate-500">Telah Dibayar</span>
                                            <span className="font-semibold text-slate-800">-{formatCurrency(project.amountPaid)}</span>
                                        </div>
                                        <div className="flex justify-between font-bold text-base sm:text-lg text-slate-900 border-t-2 border-slate-300 pt-2 mt-2">
                            ...
                            <footer className="mt-8 sm:mt-12 pt-6 sm:pt-8 border-t-2 border-slate-200">
                                {userProfile.termsAndConditions && (
                                    <div className="mb-6 sm:mb-8">
                                        ...
                                        <p className="text-xs text-slate-500 whitespace-pre-wrap leading-relaxed">
                                            {userProfile.termsAndConditions.replace(/📜|💰|⏱|📦/g, '').replace(/\s*\*\*(.*?)\*\*\s*/g, '\n<strong>$1</strong>\n')}
                                        </p>
                                    </div>
                                )}
                                <p className="text-xs text-slate-500 text-center">
                                    Jika Anda memiliki pertanyaan, silakan hubungi kami di {userProfile.phone}
                                </p>
                                <div className="w-full h-2 bg-blue-600 mt-4 sm:mt-6 rounded"></div>
                            </footer>
                        </div>
                    </div>
                </div>
            ...
            return (
                <div id="receipt-content" className="p-1">
                     <div className="printable-content bg-slate-50 font-sans text-slate-800 printable-area">
                        <div className="max-w-sm sm:max-w-md mx-auto bg-white p-4 sm:p-6 lg:p-8 shadow-lg rounded-xl">
                            <header className="text-center mb-6 sm:mb-8">
                                <h1 className="text-xl sm:text-2xl font-bold text-slate-900">KWITANSI PEMBAYARAN</h1>
                                <p className="text-xs sm:text-sm text-slate-500 mt-1">{userProfile.companyName}</p>
                            </header>
                            <div className="p-3 sm:p-4 bg-green-500/10 border border-green-200 rounded-lg text-center mb-6 sm:mb-8 printable-bg-green-light">
                                <p className="text-xs sm:text-sm font-semibold text-green-700 print-text-green">PEMBAYARAN DITERIMA</p>
                                <p className="text-2xl sm:text-3xl font-bold text-green-800 print-text-green mt-1">{formatCurrency(transaction.amount)}</p>
                            </div>
                            <div className="space-y-2 sm:space-y-3 text-xs sm:text-sm">
                                <div className="flex justify-between">
                                    <span className="text-slate-500">No. Kwitansi</span>
                                    <span className="font-semibold text-slate-700 font-mono text-xs">{transaction.id.slice(0,12)}</span>
                                </div>
                                <div className="flex justify-between">
                                ...
                                <div className="flex flex-col sm:flex-row sm:justify-between gap-1 sm:gap-0">
                                    ...
                                    <span className="font-semibold text-slate-700 break-words">{clientForDetail.name}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-slate-500">Metode</span>
                                    <span className="font-semibold text-slate-700">{transaction.method}</span>
                                </div>
                            </div>
                            <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-slate-200">
                                <p className="text-xs sm:text-sm text-slate-500">Untuk pembayaran:</p>
                                <p className="font-semibold text-slate-800 mt-1 text-sm break-words">{transaction.description}</p>
                                {project && (
                                    <div className="mt-2 text-xs text-slate-500 space-y-1">
                                        <p className="break-words">Proyek: {project.projectName}</p>
                                        <p>Total Tagihan: {formatCurrency(project.totalCost)}</p>
                                        <p>Sisa: {formatCurrency(project.totalCost - project.amountPaid)}</p>
                                    </div>
                                )}
                            </div>
                            <footer className="mt-8 sm:mt-12 flex flex-col sm:flex-row justify-between items-center sm:items-end gap-4 sm:gap-0">
                                <p className="text-xs text-slate-400">Terima kasih.</p>
                                <div className="text-center">
                                    {transaction.vendorSignature ? (
                                        <img src={transaction.vendorSignature} alt="Tanda Tangan" className="h-12 sm:h-16 object-contain mx-auto" />
                                    ) : (
                                        <div className="h-12 sm:h-16 flex items-center justify-center text-xs text-slate-400 italic border-b border-dashed min-w-[120px]">
                                            Belum Ditandatangani
                                        </div>
                                    )}
